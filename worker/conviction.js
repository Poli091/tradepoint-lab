...
function scoreRevenue(v) {
  ...
  return v>25?8 : v>=15?6 : v>=10?4 : v>=0?2 : 0
}
function scoreFCF(v) {
  if (v==null) return null
  return v>20?5 : v>=10?3 : v>=0?2 : 0
}
...
function growth(f) {
  const comps = [
    ...
    { score: scoreRevenue(f.epsGrowthYoY),     weight:8 },
    { score: scoreFCF(f.fcfGrowth5Y),           weight:5 },
    { score: acceleration(f),                   weight:4 },
  ]
  return normalize(comps, 25)
}
...
function quality(f) {
  ...
  const roicScore = roicVal==null?null : roicVal>20?8 : roicVal>=15?6 : roicVal>=10?4 : roicVal>=8?2 : 0
  const nmScore   = f.netMargin==null?null : f.netMargin>25?7 : f.netMargin>=15?5 : f.netMargin>=10?3 : f.netMargin>=0?1 : 0
  const gmScore   = f.grossMargin==null?null : f.grossMargin>60?5 : f.grossMargin>=40?3 : f.grossMargin>=20?2 : 0
  return normalize([{score:roicScore,weight:8},{score:nmScore,weight:7},{score:gmScore,weight:5}], 20)
}

// Strength (15)
function strength(f, profile) {
  if (profile.name==='banks') return { score:null, nullFields:3, skipped:true }
  const de = (f.debtToEquity!=null && f.debtToEquity<0) ? 0 : bracket(f.debtToEquity, profile.debt, 'max')
  const cr = bracket(f.currentRatio,     profile.cr, 'min')
  const ic = bracket(f.interestCoverage, profile.ic, 'min')
  return normalize([{score:de,weight:5},{score:cr,weight:5},{score:ic,weight:5}], 15)
}
...
function technical(ohlcv, spyOhlcv, currentPrice) {
  ...
  const emaScore = above==null?null : above?5:0
...
function gates(f, profile) {
  ...
  const g2checks={}
  const qOpts=[f.roic,f.roi,f.roe].filter(v=>v!=null)
  const qVal=qOpts.length>0?Math.max(...qOpts):null
  g2checks.roicOrRoe={pass:qVal==null||qVal>=8}
  g2checks.operatingMargin={pass:f.operatingMargin==null||f.operatingMargin>0}
